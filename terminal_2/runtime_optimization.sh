...
# Enhanced entrypoint script for Research Environment 2
...
setup_monitoring() {
  if [[ "$ENABLE_MONITORING" == "true" ]]; then
    echo "Setting up monitoring agent for Terminal 2"
...
import threading
def start_metrics():
    start_http_server(8889)  # Using different port for terminal_2
threading.Thread(target=start_metrics, daemon=True).start()
print('Started Prometheus metrics server on port 8889')
" &
    fi
    ...
    if [[ "$LOG_MEMORY" == "true" ]]; then
      (while true; do
        echo "[$(date)] Memory usage (Terminal 2):" >> /tmp/memory_log_terminal2.txt
        free -h >> /tmp/memory_log_terminal2.txt
        sleep 60
      done) &
...
optimize_io() {
  ...
  if [[ -n "$MEMORY_LIMIT" ]]; then
    ...
    if [[ $TMPFS_SIZE -gt 1024 ]]; then
      ...
      mkdir -p /tmp/research_2
      mount -t tmpfs -o size=${TMPFS_SIZE}m tmpfs /tmp/research_2 2>/dev/null || true
      export TMPDIR=/tmp/research_2
      
      # Set up cache directories for commonly used libraries
      mkdir -p /tmp/research_2/torch_cache
      mkdir -p /tmp/research_2/huggingface
      mkdir -p /tmp/research_2/tensorflow
      export TORCH_HOME=/tmp/research_2/torch_cache
      export TRANSFORMERS_CACHE=/tmp/research_2/huggingface
      export HF_HOME=/tmp/research_2/huggingface
      export TENSORFLOW_CACHE=/tmp/research_2/tensorflow
    fi
  fi
  
  # Same I/O optimization as terminal_1
  if [[ $EUID -eq 0 ]]; then
...
echo "=============================================="
echo "AI Research Environment 2 Configuration:"
